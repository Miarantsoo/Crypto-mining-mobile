...
import { FaArrowRightFromBracket, FaMars, FaVenus } from "react-icons/fa6";
      ...
      <IonContent fullscreen>
        ...
        {!noChange && (
          <div className="absolute w-full bottom-0 flex flex-row justify-evenly pt-20 pb-10 px-5 bg-gradient-to-b from-transparent via-black/50 to-black/80">
            <button
              className="h-12 px-5 font-title text-xl bg-light rounded-xl"
              onClick={() => setNoChange(!noChange)}
            >
              Annuler
            </button>
            <button
              className="h-12 px-5 font-title text-xl text-light bg-main rounded-xl"
              onClick={uploadImage}
            >
              Changer
            </button>
